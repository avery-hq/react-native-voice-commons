    ...
    _addCall(call) {
        ...
        if (callkit_coordinator_1.callKitCoordinator.isAvailable()) {
            ...
            if (existingCallKitUUID) {
                ...
                callkit_coordinator_1.callKitCoordinator.reportIncomingCall(telnyxCall, call.callerName, call.callerNumber);
            }
            else {
        ...
        call.callState$.subscribe((state) => {
            ...
            if (state === call_state_1.TelnyxCallState.ENDED || state === call_state_1.TelnyxCallState.FAILED) {
                ...
                voice_pn_bridge_1.VoicePnBridge.clearPendingVoipPush().catch((e) => console.warn('CallStateController: Failed to clear pending voip push:', e));
